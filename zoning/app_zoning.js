...
const bestZoneCanvas = document.getElementById("bestZoneCanvas");
const bzctx = bestZoneCanvas.getContext("2d");

const bestTemplateCanvas = document.getElementById("bestTemplateCanvas");
const btctx = bestTemplateCanvas.getContext("2d");
...
function clearZoneCanvas() {
    ...
    bzctx.fillStyle = "black";
    bzctx.fillRect(0, 0, bestZoneCanvas.width, bestZoneCanvas.height);

    btctx.fillStyle = "black";
    btctx.fillRect(0, 0, bestTemplateCanvas.width, bestTemplateCanvas.height);
}
...
function matchByZoning(inputVec) {
    ...
    const perDigitTop = Array.from(bestPerDigit.values())
        .sort((a, b) => b.score - a.score);

    const bestOverall = scored[0] ?? null;

    return { best: perDigitTop[0], perDigitTop, scored, inputFeat, bestOverall };
}
...
function recomputeAndRender() {
    ...
    const { best, perDigitTop, bestOverall } = matchByZoning(vec);

    if (bestOverall && bestOverall.t && bestOverall.t.vec) {
        drawZoningGridToCanvas(bzctx, bestZoneCanvas, bestOverall.t.vec, 28, 4);
        renderVecToCanvas(btctx, bestTemplateCanvas, bestOverall.t.vec, 28);
    } else {
        bzctx.fillStyle = "black";
        bzctx.fillRect(0, 0, bestZoneCanvas.width, bestZoneCanvas.height);
    ...
    perDigitTop.slice(0, 5).forEach((s, i) => {
